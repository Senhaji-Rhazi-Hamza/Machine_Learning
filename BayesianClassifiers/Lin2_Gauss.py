...
class Lin2_Gauss:
  def train(self, data_train, train_labels):  
    ...
    self.covinverse = np.linalg.pinv(np.cov(data_train))
 ...
 #given a test_vector(an image) return the closest label to that image
  ...
  def answer_out(self, vec_test):
    ...
    for i in range(len(self.classesAverage)):
      distances[i] = np.dot(np.dot(np.transpose(self.classesAverage[i]- vec_test), self.covinverse), (self.classesAverage[i] - vec_test))
  ...
  def performance(self, test, labels_test ):
    ...
    print("the performance of Linear2Gaussclassifier is", ret) 
    return ret
